...
function registerReportIpc() {
  ...
  ipcMain.handle(
    ...
    async (event, { customerId, dateFrom, dateTo }) => {
      try {
        const data = reportService.getCustomerLedger(
          customerId,
          dateFrom,
          dateTo,
        );
  ...
  ipcMain.handle(
    ...
    async (event, { supplierId, dateFrom, dateTo }) => {
      try {
        const data = reportService.getSupplierLedger(
          supplierId,
          dateFrom,
          dateTo,
        );
  ...
  ipcMain.handle(
    ...
    async (event, { accountId, dateFrom, dateTo }) => {
      try {
        const data = reportService.getAccountStatement(
          accountId,
          dateFrom,
          dateTo,
        );
