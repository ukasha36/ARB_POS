...
import { BarChart3, RefreshCw, AlertTriangle, XCircle, Package, DollarSign } from 'lucide-react';
import {
  useReactTable,
  getCoreRowModel,
  flexRender,
...
export function StockAnalyticsPage() {
  ...
  const categoryOptions = useMemo(() => {
    const cats = new Set(
      (analytics.categoryValuation || []).map((c) => c.category).filter(Boolean)
    );
  ...
  const loadValuation = useCallback(async (search = searchText, category = categoryFilter) => {
    ...
    () => [
      {
        ...
        header: 'Category',
        cell: (info) => (
          <span className="font-semibold text-[#1E293B]">{info.getValue() || 'Uncategorised'}</span>
        ),
      },
      {
    ...
    () => [
      {
        ...
        header: 'Item',
        cell: (info) => (
          <span className="font-semibold text-[#1E293B] leading-tight">{info.getValue()}</span>
        ),
      },
      {
        accessorKey: 'code',
        header: 'Code',
        cell: (info) => (
          <span className="font-mono text-[11px] text-[#64748B]">{info.getValue()}</span>
        ),
      },
      {
  ...
  const valuationColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Item Name',
        cell: (info) => (
          <span className="font-semibold text-[#1E293B]">{info.getValue()}</span>
        ),
      },
      {
        accessorKey: 'category',
      ...
      },
      {
        ...
        cell: (info) => {
          const qty = Number(info.getValue() || 0);
          return (
            <span className={`font-mono text-right block font-bold ${qty <= 0 ? 'text-[#DC2626]' : 'text-[#0F172A]'}`}>
              {qty.toLocaleString()}
            </span>
          );
        },
  ...
  return (
    <div className="space-y-3 select-none">
      ...
      <div className="bg-white p-3 border border-[#E2E8F0] rounded-[4px] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#64748B]">
              Category breakdown, fast movers, low stock alerts and full inventory valuation at WAC
            </p>
          </div>
        </div>
        <button
        ...
        >
          <RefreshCw className={`w-3.5 h-3.5 ${loading || valuationLoading ? 'animate-spin' : ''}`} />
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-white p-3 border border-[#E2E8F0] rounded-[4px] shadow-sm bg-[#EFF6FF]/40">
          <div className="flex items-center justify-between text-[#2563EB] mb-1">
            <span className="text-[10px] font-bold uppercase tracking-wider">Total Valuation</span>
        ...
        <div className="bg-white p-3 border border-[#E2E8F0] rounded-[4px] shadow-sm">
          ...
          <span className="text-[10px] text-[#94A3B8]">Below minimum level</span>
        </div>
        ...
        <div className="bg-white p-3 border border-[#E2E8F0] rounded-[4px] shadow-sm">
          ...
          <span className="text-[10px] text-[#94A3B8]">Zero or negative qty</span>
        </div>
      </div>
                ...
                <tbody className="divide-y divide-[#E2E8F0]">
                  {lowStockTable.getRowModel().rows.length > 0 ? (
                    lowStockTable.getRowModel().rows.map((row) => {
                      ...
                      const rowBg = qty <= 0 ? 'bg-[#FFF1F2]' : 'bg-[#FFFBEB]/60';
                      return (
                        <tr key={row.id} className={`${rowBg} hover:brightness-[0.97] transition`}>
                          {row.getVisibleCells().map((cell) => (
                            <td key={cell.id} className="px-3 py-2 border-r border-[#E2E8F0] last:border-r-0 text-[#0F172A]">
                              {flexRender(cell.column.columnDef.cell, cell.getContext())}
                            </td>
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-[4px] overflow-hidden">
        ...
        >
          <div className="flex-1">
            <label className="block text-[10px] font-bold text-[#475569] uppercase mb-1">Search Item</label>
            <input
              type="text"
              value={searchText}
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead className="bg-[#F8FAFC] border-b border-[#E2E8F0] sticky top-0 z-10">
              {valuationTable.getHeaderGroups().map((hg) => (
                <tr key={hg.id}>
                  {hg.headers.map((h) => (
                    <th key={h.id} className="px-3 py-2 font-bold text-[#475569] text-[11px] uppercase tracking-wider border-r border-[#E2E8F0] last:border-r-0 whitespace-nowrap">
                      {flexRender(h.column.columnDef.header, h.getContext())}
                    </th>
                  ))}
                </tr>
            ...
            <tbody className="divide-y divide-[#E2E8F0]">
              {valuationLoading ? (
                <tr>
                  <td colSpan={valuationColumns.length} className="px-3 py-8 text-center text-[#94A3B8]">
        ...
        {!valuationLoading && valuation.items.length > 0 && (
          <div className="px-3 py-1.5 border-t border-[#E2E8F0] bg-[#F8FAFC] flex items-center justify-between">
            <span className="text-[10px] text-[#94A3B8]">
              Showing {valuation.items.length.toLocaleString()} item{valuation.items.length !== 1 ? 's' : ''}
            </span>
