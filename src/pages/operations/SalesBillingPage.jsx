...
export function SalesBillingPage() {
  ...
  const handleLineItemChange = (index, field, value) => {
    ...
    const disc = safeNum(row.discount, 0);
  ...
  const handleBarcodeSearch = (e) => {
    ...
    const matchedItem = availableItems.find(
      (i) =>
        i.barcode === barcodeQuery.trim() ||
        i.code.toLowerCase() === barcodeQuery.trim().toLowerCase(),
    );

    if (matchedItem) {
      // Check if item already in line items -> increment qty
      const existingIdx = lineItems.findIndex(
        (li) => li.item_id === matchedItem.id,
      );
      if (existingIdx >= 0) {
        handleLineItemChange(
          existingIdx,
          "qty",
          lineItems[existingIdx].qty + 1,
        );
      } else {
        setLineItems([
          ...lineItems,
      ...
      setStatus({
        type: "error",
        text: `Item barcode/code '${barcodeQuery}' not found.`,
      });
    }
  };
  ...
  const grandTotal =
    Math.round(
      lineItems.reduce((sum, item) => sum + safeNum(item.total, 0), 0) * 100,
    ) / 100;
  const numPaid = Math.min(grandTotal, Math.max(0, safeNum(paidAmount)));
  ...
  const handlePostSale = async (e) => {
    ...
    if (numPaid > 0 && !depositAccountId) {
      setStatus({
        type: "error",
        text: "Please select a Cash/Bank Account for payment.",
      });
      return;
    }
    ...
    if (cust && creditBalance > 0 && cust.credit_limit > 0) {
      ...
      if (creditBalance > cust.credit_limit) {
        setStatus({
          type: "error",
          text: `Credit sale of ${formatCurrency(creditBalance)} exceeds customer's credit limit of ${formatCurrency(cust.credit_limit)}.`,
        });
        return;
      }
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        setStatus({
          type: "error",
          text: res.error || "Failed to post sales billing",
        });
      }
    } catch (err) {
  ...
  const handleEditSale = async (tx) => {
    try {
      ...
      if (res.success && res.data) {
        ...
        const salesCredit = creditLines.find(
          (l) =>
            l.account_type === "REVENUE" ||
            l.account_type === "SALES" ||
            l.account_type === "INCOME",
        );
        if (salesCredit)
          setSalesRevenueAccountId(safeId(salesCredit.account_id));
        ...
        const customerDebit = debitLines.find(
          (l) => l.account_type === "CUSTOMER",
        );
        ...
        const cashPayment = debitLines.find(
          (l) => l.account_type === "CASH" || l.account_type === "BANK",
        );
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-xs font-bold text-[#0F172A]">
              SALES COUNTER POS BILLING
            </h3>
            <p className="text-[11px] text-[#64748B]">
              Fast cashier billing screen with barcode support, cash receipt,
              and stock deduction
            </p>
          </div>
        </div>
            ...
            >
              {customers.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.code} - {c.title}{" "}
                  {c.credit_limit > 0 ? `(Limit: ${c.credit_limit})` : ""}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="col-span-3">
            ...
            {availableRevenueAccounts.length === 0 ? (
              ...
              >
                ...
                {availableRevenueAccounts.filter(Boolean).map(
                  (acc, index) =>
                    acc && (
                      <option
                        key={safeId(acc?.id) || `rev-${index}`}
                        value={safeId(acc?.id)}
                      >
          ...
          {availableItems.length === 0 ? (
            <div className="p-6 text-center">
              ...
              <p className="text-[11px] text-[#64748B] mt-1">
                Setups → Items mein product banao, phir yahan sale karo.
              </p>
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {lineItems.map((item, idx) => {
                  const selectedItem = availableItems.find(
                    (ai) => ai.id === parseInt(item.item_id, 10),
                  );
                  return (
                    <tr key={idx} className="border-b border-[#E2E8F0]">
                      ...
                      <td className="p-1 border-r border-[#E2E8F0]">
                        ...
                        {selectedItem && (
                          <div
                            className={`text-[10px] font-bold px-2 ${selectedItem.stock_qty <= 0 ? "text-red-500" : "text-green-600"}`}
                          >
        ...
        <div className="bg-white p-3 border border-[#E2E8F0] rounded-[4px] grid grid-cols-12 gap-3 items-center">
          ...
          <div className="col-span-7 bg-[#F8FAFC] p-2 border border-[#E2E8F0] rounded-[3px] grid grid-cols-3 gap-2 text-right">
            ...
            <div>
              ...
              <span className="font-mono text-base font-bold text-[#DC2626]">
        ...
        >
          <div className="p-4">
            <p className="text-xs text-[#475569] mb-2">
              Void sales invoice #{voidConfirm.tx.reference_no}? This reverses
